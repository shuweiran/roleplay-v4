...
async function scenarioScene2d(outDir) {
  ...
  try {
    await nav(cdp, 'http://localhost:8000/');
    await login(cdp);
...
async function scenarioChat2d(outDir) {
  ...
  try {
    ...
    await waitFor(`document.querySelector('.chat-main') !== null && document.querySelector('.panel-werewolf') !== null`, 150000, '剧本生成+进入对局');
    await sleep(3000);
    ...
    await waitFor(`!![...document.querySelectorAll('button')].find(b=>(b.textContent||'').includes('查看 2D 模拟'))`, 45000, '讨论启动/2D 按钮出现');
    await sleep(1500);
